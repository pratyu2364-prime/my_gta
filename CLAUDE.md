...
| Mouse camera | `camYaw`, `camPitch`, `pointerLocked`, `lastMouse` | Click canvas → pointer lock; `mousemove` adjusts yaw/pitch. Camera orbits at `player.heading+camYaw`. Auto-recenters behind you when driving & mouse idle >1.5s. **Aim direction = `player.heading+camYaw`** (so you shoot where you look). Headless has no lock → camYaw stays 0 → classic chase cam. |
